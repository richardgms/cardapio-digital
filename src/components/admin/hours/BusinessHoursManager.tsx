...
import { Loader2, Plus, Trash2, Clock, Info, Save } from "lucide-react";
    ...
    const loadData = async () => {
        try {
            ...
            if (isImpersonating && storeId) {
                // Via Super Admin, buscamos usando o adminClient indiretamente ou via proxy-hours fetch (simplificando aqui pois store_config e publico/proprio no lido)
                // Mas para consistencia, o Super Admin ja tem acesso a store_config do lojista via RLS bypass no withSuperAdmin
                // Vamos buscar direto aqui (assumindo que o Supervisor consegue ler se houver RLS permissivo ou via service role num wrapper)
                // Para manter o padrao, vamos usar o supabase client se estiver logado como admin ou buscar via action.
    ...
    const onSave = async () => {
        ...
        try {
            if (isImpersonating && storeId) {
                ...
                // Lógica de fallback para lojista (pode extrair pra action depois)
                const { data: { user } } = await supabase.auth.getUser();
    ...
    if (isLoading) {
        return (
            <div className="space-y-6 pt-6">
                <Skeleton className="h-20 w-full rounded-2xl bg-zinc-100" />
                <Skeleton className="h-[400px] w-full rounded-2xl bg-zinc-100" />
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight text-black">Horários</h1>
                    <p className="text-muted-foreground mt-1 text-sm">
                        {isImpersonating ? "Configurando horários como Super Admin." : "Gerencie os horários de funcionamento da sua loja."}
                    </p>
                </div>
                <Button onClick={onSave} disabled={isSaving} className="bg-black text-white hover:bg-zinc-800">
                    <Save className="mr-2 h-4 w-4" /> {isSaving ? "Salvando..." : "Salvar Alterações"}
                </Button>
            </div>

            <Card className="border border-zinc-200 bg-zinc-50/50 shadow-none rounded-2xl">
                <CardContent className="pt-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-0.5">
                            <label htmlFor="auto-mode" className="font-bold text-black cursor-pointer">
            ...
            {!autoEnabled ? (
                <div className="py-20 text-center border-2 border-dashed border-zinc-200 rounded-2xl bg-zinc-50/20">
                    <Info className="w-10 h-10 mx-auto text-zinc-300 mb-4" />
                    <h3 className="text-lg font-bold text-zinc-400">Controle Manual Ativo</h3>
                ...
                <div className="grid gap-4">
                    {daysConfig.map((day) => (
                        <div key={day.day_of_week} className="flex flex-col md:flex-row md:items-center justify-between p-6 bg-white border border-zinc-200 rounded-2xl shadow-sm gap-4">
                            <div className="flex items-center gap-4 min-w-[200px]">
                                <Switch
                                    ...
                                    className="data-[state=checked]:bg-black"
                                />
                                <span className={cn("font-bold text-lg", !day.is_open && "text-zinc-300")}>
                            ...
                            {day.is_open ? (
                                <div className="flex-1 flex flex-wrap items-center gap-3">
                                    {day.periods.map((period, pIndex) => (
                                        <div key={pIndex} className="flex items-center bg-zinc-50 px-3 py-2 rounded-xl border border-zinc-100 gap-2 animate-in fade-in zoom-in duration-200">
                                            <Input
                                                type="time"
                                            ...
                                            />
                                            <span className="text-zinc-400 font-bold text-xs">ATÉ</span>
                                            <Input
                                                type="time"
                                                ...
                                                className="w-[90px] h-8 border-none bg-transparent font-medium p-0 focus-visible:ring-0"
                                            />
                                            <button onClick={() => handleRemovePeriod(day.day_of_week, pIndex)} className="text-zinc-300 hover:text-black">
                                                <X className="w-4 h-4" />
                                            </button>
                                        </div>
                                    ))}
                                    <Button variant="ghost" size="sm" onClick={() => handleAddPeriod(day.day_of_week)} className="text-zinc-400 hover:text-black font-bold h-10 px-4 border border-dashed border-zinc-200 rounded-xl">
                                        <Plus className="w-4 h-4 mr-1" /> ADICIONAR
                                    </Button>
                                </div>
                            ) : (
                                <div className="flex-1 text-right">
                                    <span className="text-xs font-bold text-zinc-300 uppercase tracking-widest">Fechado</span>
                                </div>
                            )}
                        </div>
                    ))}
